...
const UpdateAccount: React.FC = () => {
  ...
  const updateUserData = async (data: string, type: string): Promise<void> => {
    ...
    const params =
      type === "phone"
        ? { phoneNumber: getValues("phoneNumber"), otp: data }
        : { email: getValues("email"), otp: data };
    try {
    ...
    <IonPage style={isPlatform("ios") ? { paddingTop: 40 } : { paddingTop: 0 }}>
      <IonContent>
                        ...
                        name === "email"
                          ...
                          ? {
                              minLength: {
                                value: 10,
                                message: t(
                                  "Phone numbers with minimum is 10 digits"
                                ),
                              },
                              maxLength: {
                                value: 11,
                                message: t(
                                  "Phone numbers with up to 11 digits"
                                ),
                              },
                              pattern: {
                                value: /^[0-9\b]+$/,
                      ...
                      render={({ field: { onChange, onBlur, value } }) => (
                        <IonRow>
                          ...
                          <IonCol size="12">
                            ...
                            {errors?.email?.message && name === "email" && (
        ...
        <IonAlert
          ...
          message={
            success
              ? t("Update account successfully")
              : t("Incorrect code! Update account failed")
          }
          buttons={[
            {
