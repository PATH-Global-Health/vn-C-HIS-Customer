...
const ResultExaminations: React.FC = () => {
  ...
  const resultListTesting = examinationList.data.filter(e => e.status === 6 && e.service.id === 'f2490f62-1d28-4edd-362a-08d8a7232229');
  const resultListAdvise = examinationList.data.filter(e => e.status === 6 && e.service.id === '9f9e8dd3-890e-4ae5-2952-08d92b03ae12');
  const resultListSearch = resultList.filter(e => deburr(e.service.name + e.unit.name + e.unit.address + e.date + e.interval.from).includes(deburr(nameSearch)));
      ...
      <IonPage className={styles.styledPage}>
        ...
        {searchInput === false ?
          <IonHeader className={styles.header}>
            ...
            <IonLabel className={styles.headerLabel}>{t('Result information')} </IonLabel>
            <IonIcon
            ...
            <ul className={styles.styledMenu}>
              <li
                ...
                onClick={() => setSelectedMenu("test")}>{t('Test')}</li>
              <li
                className={cx({ 'styledLiSelected': selectedMenu === "advise" })}
                onClick={() => setSelectedMenu("advise")}>{t('Advise')}</li>
            </ul>
          </IonHeader>
            ...
            <>
              ...
              {resultListSearch.map((e) =>
                <div className={styles.styledCardDiv}>
                  <IonCard
                    onClick={() => {
                      history.push({ pathname: '/examinationResultInfo', state: e.id })
                    }}
                    className={cx('styledCardAids', {
                      'styledCardBlood': e.service.id === "f2490f62-1d28-4edd-362a-08d8a7232229",
                      'styledCardNegative': e.result === "Âm tính" && e.service.id === "f2490f62-1d28-4edd-362a-08d8a7232229",
                      'styledCardAdvise': e.service.id === "9f9e8dd3-890e-4ae5-2952-08d92b03ae12",
                    })}
                  >
                    <div
                    ...
                    >
                      <IonCardTitle
                        className={cx('styledCardTitleBlood', { 'styledCardTitleBlood': e.service.id === "f2490f62-1d28-4edd-362a-08d8a7232229" })}
                      >{e.service.name}
                      </IonCardTitle>
                      <IonCardSubtitle
                        className={cx('styledSubtitleBlood', { 'styledSubtitleBlood': e.service.id === "f2490f62-1d28-4edd-362a-08d8a7232229" })}
                      >Vào lúc {e.interval.from}, {moment(e.date).format("DD-MM-YYYY")}</IonCardSubtitle>
                    </IonCardHeader>
                    <IonCardContent
                      className={cx('styledCardContentBlood', { 'styledCardContentBlood': e.service.id === "f2490f62-1d28-4edd-362a-08d8a7232229" })}
                    >
                      <p>{e.unit.name}</p>
                      <p>{e.unit.address}</p>
                    </IonCardContent>
                  ...
                  }} className={styles.btnCancelCard}><IonIcon style={{ fontSize: '25px' }} icon={cloudDownloadOutline} /></button>
                ...
                <div className={styles.styledDivHistory}>
                  {resultListAdvise.map(e =>
                    <div className={styles.styledCardDiv}>
                      ...
                      >
                        <div
                          className={cx('styledDivIconAids', { 'styledDivIconAids': e.service.id !== "f2490f62-1d28-4edd-362a-08d8a7232229" })}
                        >
                          <IonIcon className={styles.styledIcon} icon={e.service.id === "f2490f62-1d28-4edd-362a-08d8a7232229" ? analytics : chatbubbles}></IonIcon>
                        </div>
                        <IonCardHeader
                      ...
                      <button onClick={() => {
                        examinationServices.downloadResultFile(e.id);
                      }} className={styles.btnCancelCard}><IonIcon style={{ fontSize: '25px' }} icon={cloudDownloadOutline}></IonIcon></button>
