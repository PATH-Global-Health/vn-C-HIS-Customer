...
const UpdateLaytest: React.FC = () => {
  ...
  const onSubmit = async (data: any): Promise<void> => {
    try {
      await LaytestService.updateLaytest({
        id: data.id,
        resultTesting: data.result?.resultTesting,
      });
      setSuccess(true);
      setShowAlert(true);
    } catch (error) {
    ...
    <IonPage style={isPlatform("ios") ? { paddingTop: 40 } : { paddingTop: 0 }}>
      ...
      <IonContent>
        ...
        <IonAlert
          ...
          message={
            success
              ? t("Update result laytest successfully")
              : t("Update result laytest failed")
          }
          buttons={[
            {
